...
/**
 * Этот класс считает данные из запроса и отправляет их обратно.
 */
	...
	public void start() {
		...
		try (ServerSocket server = new ServerSocket(9000)) {
			while (!server.isClosed()) {
				...
				pool.execute(() -> {
					try (OutputStream out = socket.getOutputStream(); InputStream input = socket.getInputStream()) {
						...
						var request = Request.of(content);
						var resp = modes.get(request.getPoohMode()).process(request);
